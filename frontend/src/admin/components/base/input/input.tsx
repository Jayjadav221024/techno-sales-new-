import { type ComponentType, type HTMLAttributes, type ReactNode, type Ref, createContext, useContext, useState, isValidElement } from "react";
...
export const InputBase = ({
    ...
    return (
        ...
        >
            ...
            {Icon && (() => {
                const IconComponent = typeof Icon === 'function' ? Icon : (Icon?.default || Icon?.Mail01 || Icon?.Lock01 || Icon);
                if (typeof IconComponent === 'function') {
                    return <IconComponent className={cx("pointer-events-none absolute text-fg-quaternary", sizes[inputSize].iconLeading, context?.iconClassName, iconClassName)} />;
                }
            ...
            {tooltip && type !== "password" && (() => {
                const HelpCircleComp = typeof HelpCircle === 'function' ? HelpCircle : (HelpCircle?.HelpCircle || HelpCircle?.default);
                return (
                    <Tooltip title={tooltip} placement="top">
                        ...
                        >
                            {HelpCircleComp && <HelpCircleComp className="size-4 stroke-[2.25px]" />}
                        </TooltipTrigger>
                    </Tooltip>
                );
            })()}
            ...
            {/* Invalid icon */}
            {type !== "password" && (() => {
                const InfoCircleComp = typeof InfoCircle === 'function' ? InfoCircle : (InfoCircle?.InfoCircle || InfoCircle?.default);
                return InfoCircleComp ? (
                    <InfoCircleComp
                        className={cx(
            ...
            {type === "password" && (() => {
                const EyeOffComp = typeof EyeOff === 'function' ? EyeOff : (EyeOff?.EyeOff || EyeOff?.default);
                const EyeComp = typeof Eye === 'function' ? Eye : (Eye?.Eye || Eye?.default);
                return (
                    <AriaButton
                    ...
                    >
                        {isPasswordVisible ? (
                            EyeOffComp && <EyeOffComp className="size-4 stroke-[2.25px]" />
                        ) : (
                            EyeComp && <EyeComp className="size-4 stroke-[2.25px]" />
                        )}
                    </AriaButton>
                );
            })()}
