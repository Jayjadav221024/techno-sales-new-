...
export default function ThreeModalCanvas({ category }) {
  ...
  useEffect(() => {
    ...
    if (cat.includes('motor')) {
      ...
      let mainColor = 0x1e293b;
      let finColor = 0x00e5ff;

      if (cat.includes('siemens')) {
        mainColor = 0x004d61; // Siemens dark teal
        finColor = 0x00e5ff;   // Cyan
      } else if (cat.includes('cg')) {
        mainColor = 0x14532d; // CG dark green
        finColor = 0x22c55e;   // Green
      } else if (cat.includes('abb')) {
        mainColor = 0x7f1d1d; // ABB dark red
        finColor = 0xef4444;   // Red
      }
